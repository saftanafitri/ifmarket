...
@section('title', 'Sakti Product - Tambah Produk')
    ...
    <form id="addProductForm" action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control border-warning" id="name" name="name" placeholder="Input"
                value="{{ old('name') }}" 
                data-check-url="{{ route('products.checkName') }}" required>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select border-warning" id="category_id" name="category_id" required>
                        {{-- Tambahkan disabled dan selected pada option pertama --}}
                        <option value="" disabled selected hidden>Pilih kategori</option>
                        @foreach ($categories as $items)
                            {{-- Perbaiki logika untuk old value --}}
                            <option value="{{ $items->id }}" {{ old('category_id') == $items->id ? 'selected' : '' }}>
                                {{ $items->name }}
                            </option>
                        @endforeach
                    </select>
