            ...
            @if (!in_array(Route::currentRouteName(), ['login', 'products.detail', 'products.create', 'manageProduct', 'products.show']))
                <div class="input-group w-50">
                    <form action="{{ route('products.search') }}" method="GET" class="d-flex w-100">
                        <input type="text" name="query" class="form-control" placeholder="Search for anything..." value="{{ request('query') }}">
            ...
            <div class="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="profileDropdown">
                    @if(!Auth::check() && Route::currentRouteName() == 'home.index')
                        <!-- Jika belum login dan sedang di halaman Home, hanya tampilkan opsi Login -->
                        <li><a class="dropdown-item" href="{{ route('login') }}">Login</a></li>
                    @else
                        <!-- Tampilkan opsi Home jika pengguna sudah login atau tidak sedang di halaman Home -->
